...
export default function EnhancedDataTable({ title, description, data }: DataTableProps) {
  ...
  const sortedData = [...data].sort((a, b) => {
    ...
    // Handle responsible field (can be string or array)
  ...
  // Calculate selected rows totals
  const selectedTotal2024 = Array.from(selectedRows).reduce((sum, rowId) => {
    const row = data.find(r => r.id === rowId);
    return sum + (row?.year2024 || 0);
  }, 0);
  const selectedPercentage = total2024 > 0 ? (selectedTotal2024 / total2024) * 100 : 0;
  ...
  return (
    <>
      <div className="w-full max-w-[1200px]">
        ...
        {/* Selection Summary */}
        {selectedRows.size > 0 && (
          <div className="bg-[#f5f5f5] border border-[#e5e5e5] rounded-[8px] p-[10px] mb-[12px] flex items-center gap-[16px]">
            <p className="font-['Geist:Medium',sans-serif] text-[13px] text-[#0a0a0a]">
              Total sélectionné (2024): <span className="font-semibold">{selectedTotal2024.toFixed(1)} Md€</span>
            </p>
            <p className="font-['Geist:Regular',sans-serif] text-[13px] text-[#626262]">
              ({selectedPercentage.toFixed(1)}% du total)
            </p>
          </div>
        )}
        ...
        <div className="w-full overflow-x-auto">
          ...
          {sortedData.map((row, index) => (
            ...
            >
              {/* Checkbox */}
              <div className="content-stretch flex h-[44px] items-center justify-center relative shrink-0 w-[40px]">
                <input
                  type="checkbox"
